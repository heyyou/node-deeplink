function deepLink(options) {
  ...
  var openStore = options.open_store !== 'false';
  ...
  var urls = {
    deepLink: url,
    iosStoreLink: openStore ? iosStoreLink : null,
    android_intent:
      'intent://' +
      path +
      '#Intent;scheme=' +
      ...
      androidPackageName +
      ';end;',
    playStoreLink: openStore ? playStoreLink : null,
    fallback: fallback
  };
  ...
  function iosLaunch() {
    var matches = ua.match(/Version\/(\d){1,}/);
    var version = parseInt(matches ? matches[0].substring('Version/'.length) : '0');

    // chrome and safari on ios >= 9 don't allow the iframe approach
    if (ua.match(/CriOS/) || (ua.match(/Safari/) && version >= 9)) {
      // on ios <= 12, it doesn't automatically open the app store so we redirect them to ios store link or fallback
      if (version <= 12) {
        window.location.replace(urls.iosStoreLink || urls.fallback);
        return;
      }
